...
# Usage:
#   ./launch_all.sh [dongtan|office|isu]
# 예:
#   ./launch_all.sh dongtan
#   ./launch_all.sh office
#   ./launch_all.sh isu
#
# 스크립트는 PUB_ENV 에 따라 pub_* 스크립트를 실행하고,
# 그와 동일하게 사용해야 할 PCD 경로를 자동으로 매핑해
# Python에 --pcd_map_path 인자로 전달합니다.
...
cd /workspace/third_party/glim/glim_ws
source /workspace/third_party/glim/glim_ws/install/setup.bash
