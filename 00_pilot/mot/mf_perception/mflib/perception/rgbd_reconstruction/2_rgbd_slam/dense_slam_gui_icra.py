...
import threading
import time
from common import load_rgbd_file_names, save_poses, load_intrinsic, extract_trianglemesh, get_default_dataset, extract_rgbd_frames, extract_pointcloud


EXCEPTION_RULE_SKIP = 0
EXCEPTION_RULE_STOP_PROCESSING = 1
exception_rule = EXCEPTION_RULE_STOP_PROCESSING
stop_processing_idx = 5000
START_IDX = 10
IDX_STEP_SIZE = 3
...
class ReconstructionWindow:
    def __init__(self, config, font_id):
        ...
        self.fixed_prop_grid.frame = gui.Rect(0, 0, 600, 0)  # 가로 길이 600 설정
        ...
        self.voxel_size_slider.set_limits(0.001, 0.02)
        ...
        self.est_block_count_slider.set_limits(40000, 1000000)
        ...
        self.est_point_count_slider.set_limits(500000, 80000000)
        ...
        self.interval_slider.int_value = 10
        ...
        self.max_slider.set_limits(0.1, 6.0)
    ...
    def _on_close(self):
        ...
        if self.is_started:
            ...
            mesh = extract_trianglemesh(self.model.voxel_grid, config,
                                        mesh_fname)
            pcd_name = '.'.join(config.path_npz.split('.')[:-1]) + '.pcd'
            print('Saving pcd to {}...'.format(pcd_name))
            pcd = extract_pointcloud(self.model.voxel_grid, config, pcd_name)
    ...
    def update_main(self):
        # open pose
        poses = []
        with open(os.path.join(config.path_dataset, 'tf_synced.txt'), 'r') as f:
            lines = f.readlines()
            for line in lines: 
                tf_mat = line.split(',')[1:]
        ...
        #     poses[i] = np.linalg.inv(init_tf) @ poses[i]
        ...
        tf_imu_to_cam = np.array([
            [0.7071067811865476, 0, -0.7071067811865476, 0], 
            [0, 1, 0, 0],
            [0.7071067811865476, 0, 0.7071067811865476, 0],
            [0, 0, 0, 1]
        ])
        T_frame_to_model = poses[0] @ tf_imu_to_cam
        ...
        while not self.is_done:
            if not self.is_started or not self.is_running:
                time.sleep(0.05)
                continue
            if self.idx >= stop_processing_idx:
                self.is_done = True
                self.is_running = False
                break
            ...
            T_frame_to_model = poses[self.idx] @ tf_imu_to_cam
            self.poses.append(T_frame_to_model)
            ...
            try:
                ...
                print('Failed to integrate frame {}'.format(self.idx))
                if exception_rule == EXCEPTION_RULE_STOP_PROCESSING:
                    self.is_done = True
                    self.is_running = False
                    break
                elif exception_rule == EXCEPTION_RULE_SKIP:
                    self.idx += IDX_STEP_SIZE
                    continue
                else:
            ...
            frustum = o3d.geometry.LineSet.create_camera_visualization(
                ...
                np.linalg.inv(T_frame_to_model), 0.2)
            ...
            info += 'Transformation:\n{}\n'.format(
                ...
                np.array2string(T_frame_to_model,
                                precision=3,
                                max_line_width=40,
                                suppress_small=True))
...
if __name__ == '__main__':
    ...
    parser.add('--path_dataset',
               help='path to dir which include rgb, depth and synchronized 6DOF pose',
               required=True)
    parser.add('--path_npz',
               help='path to the npz file that stores voxel block grid.',
               default='../data/step2_out/output/output.npz')
