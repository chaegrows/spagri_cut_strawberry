...
if [ ! -d "$COMMON_PATH" ]; then
    ...
    echo "Please download the 'common' repository and place it in the correct directory."
    return 1
fi
...
docker run -it --rm \
    ...
    -v ${PROJECT_PATH}/docker/entrypoint.sh:/entrypoint.sh \
    ${DOCKER_IMAGE} \
    /entrypoint.sh \
    /bin/bash
