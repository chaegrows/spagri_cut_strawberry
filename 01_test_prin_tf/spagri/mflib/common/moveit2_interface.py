...
from typing import Type, Any, List, Optional, Tuple, Union
import time
...
class MoveIt2Interface:
    ...
    def __init__(
        ...
        self._node = node
        self.rate = self._node.create_rate(10)
    ...
    def move_to_pose(
        ...
        if self.__use_move_group_action and not cartesian:
            ...
            # 카르테시안 경로 계획 시 max_cartesian_speed가 적용되도록 설정
            if cartesian:
                # max_cartesian_speed 값을 move_action_goal에 설정
                if hasattr(self.__move_action_goal.request, "max_cartesian_speed"):
                    self._node.get_logger().info(
                        f"Setting max_cartesian_speed to {self.max_cartesian_speed}"
                    )
    ...
    def plan_async(
        ...
        max_wait_attempts = 50  # 최대 5초 대기 (0.1초 x 50)
        wait_attempts = 0
        while start_joint_state is None:
            if wait_attempts >= max_wait_attempts:
                self._node.get_logger().error(
                    "Timeout waiting for joint states. Cannot proceed with planning."
                )
                return None

            if self.__joint_state is not None:
                start_joint_state = self.__joint_state
                self._node.get_logger().info("Joint states are available now")
                break
            else:
                if wait_attempts == 0:
        ...
        if cartesian:
            # 카르테시안 경로 계획 시 max_cartesian_speed 설정 확인
            if hasattr(self.__move_action_goal.request, "max_cartesian_speed"):
                self._node.get_logger().info(
                    f"Using max_cartesian_speed: {self.max_cartesian_speed} for cartesian planning"
                )
    ...
    def execute(self, joint_trajectory: JointTrajectory):
        ...
        if joint_trajectory is not None:
            joint_trajectory = self._apply_velocity_scaling(joint_trajectory)
    ...
    def wait_until_executed(self) -> bool:
        ...
        while self.__is_motion_requested or self.__is_executing:
          self.rate.sleep()
          self._node.get_logger().warn(f"Waiting until motion is executed: {self.__is_motion_requested} {self.__is_executing} {self.motion_suceeded}")
          if self.__is_motion_requested and not self.__is_executing and time.time() - t_start > 5.0:
            self._node.get_logger().error(f"@@@@@Motion is not executed after 5 seconds. Setting motion_suceeded to False")
    ...
    def _plan_cartesian_path(
        ...
        # 추가: max_cartesian_speed 설정 (있다면)
        if hasattr(self.__cartesian_path_request, "max_cartesian_speed"):
            self.__cartesian_path_request.max_cartesian_speed = (
                self.__move_action_goal.request.max_cartesian_speed
            )
    ...
    def __init_move_action_goal(
        ...
        # 추가: max_cartesian_speed 초기화
        if hasattr(move_action_goal.request, "max_cartesian_speed"):
    ...
    def _apply_velocity_scaling(self, joint_trajectory: JointTrajectory) -> JointTrajectory:
        """
        Apply velocity and acceleration scaling to the joint trajectory.
        """
        if not joint_trajectory.points:
            return joint_trajectory

        # 복사본 생성
        scaled_trajectory = copy.deepcopy(joint_trajectory)

        # 속도 및 가속도 스케일링 팩터 가져오기
        velocity_scaling = self.max_velocity
        acceleration_scaling = self.max_acceleration

        self._node.get_logger().info(
            f"Applying velocity scaling: {velocity_scaling}, acceleration scaling: {acceleration_scaling}"
        )
        ...
        for point in scaled_trajectory.points:
            ...
            # 시간 스케일링 (속도가 줄어들면 시간은 늘어남)
            if velocity_scaling > 0:
                # 전체 시간을 nanoseconds로 변환
                total_nanosec = point.time_from_start.sec * 1_000_000_000 + point.time_from_start.nanosec
                # 스케일링 적용
                scaled_nanosec = int(total_nanosec / velocity_scaling)
                # 다시 sec와 nanosec로 분리
                point.time_from_start.sec = scaled_nanosec // 1_000_000_000
                point.time_from_start.nanosec = scaled_nanosec % 1_000_000_000
