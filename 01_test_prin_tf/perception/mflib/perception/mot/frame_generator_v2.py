...
class FrameGeneratorV2:
  ...
  def sync_rgbd(self, rgb_queue, depth_queue, **kwargs):
    # return (SUCCESS, frame) or
    # (FAILURE, reason)
    # assume queue is filled with ascending order of timestamp

    # kwargs
    max_allowed_rgbd_timediff_ms = kwargs.get('max_allowed_rgbd_timediff_ms', 10) # ms
  ...
  def sync_rgbd_odom(self, rgb_queue, depth_queue, odom_queue, **kwargs):
    ...
    is_success, matched_rgbd \
      = self.sync_rgbd(rgb_queue, depth_queue, squeeze=False)
    if not is_success:
