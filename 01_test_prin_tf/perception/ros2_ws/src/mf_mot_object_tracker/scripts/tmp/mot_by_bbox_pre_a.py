#! /usr/bin/env python3
# -*- coding: utf-8 -*-

import rclpy
import os
import numpy as np
import cv2
import time
import sys
import json

from cv_bridge import CvBridge
from sensor_msgs.msg import Image, CompressedImage, CameraInfo
from mf_perception_msgs.msg import YoloOutArray
from tf2_ros.buffer import Buffer
from tf2_ros.transform_listener import TransformListener
from threading import Lock
from nav_msgs.msg import Odometry
from scipy.spatial.transform import Rotation
import copy
from collections import defaultdict
from rclpy.qos import QoSProfile, DurabilityPolicy, ReliabilityPolicy, HistoryPolicy
from tf2_msgs.msg import TFMessage

from mflib.perception.mot import common as mot_common
from mflib.perception.mot import datatypes as mot_dtypes
from mflib.perception.mot import frame_generator_v2 as mot_frame_generator
from mflib.perception.mot import frame_algorithms as m_fa
from mflib.perception.utils import mot3d_visualizer
from mflib.perception.utils import manipulator_adapter
from visualization_msgs.msg import Marker, MarkerArray

from config.common.leaves.leaf_actions import LeafActionLookup
from mflib.common.behavior_tree_impl_v2 import BehaviorTreeServerNodeV2
from mflib.common.behavior_tree_context import BehaviorTreeContext
from mf_msgs.msg import BehaviorTreeStatus
from config.common.specs.farm_def import Sector
from config.common.specs.work_def import ALLOWED_WORKS
from typing import Optional, Dict
from mflib.common.tf import ROS2TF
from mflib.common.mf_base import raise_with_log




if sys.argv[1] == 'strawberry_harvesting_pollination':
  import params.mot_by_bbox.strawberry_harvesting_pollination as P


class LoggerReprojectedTargets:
  def __init__(self, mot3d_visualizer, logger):
    self.frame_cached = None
    self.target_cached = {}

    self.frame = None
    self.target_trajectories = []

    self.sector_cnt_dict = defaultdict(int)

    self.mot3d_visualizer = mot3d_visualizer
    self.logger = logger

    self.image_save_dir = None

  def set_target_cached(self, frame_id, xyz_global_frame):
    self.target_cached[frame_id] = xyz_global_frame

  def set_frame_cached(self, frame_cached):
    self.frame_cached = frame_cached
  
  def set_frame(self):
    self.frame = copy.copy(self.frame_cached)

  def add_target(self, target_frame_id):
    traj = self.target_cached.get(target_frame_id, None)
    if traj is not None:
      self.target_trajectories.append(traj)
    else:
      self.logger.error(f'No target cached for {target_frame_id}. Please check the mot_by_bbox_bt_server_node.py')
      raise_with_log(self.logger, ValueError, 'Impossible control flow happen. I assume target_frame_id from query_target_frame_id is fed into this function')
  
  def save_reprojected_image(self, sector, dir_name, trajectory_set):
    if self.frame is None:
      self.logger.error('No frame for reprojected image saving(). Abort...')
      return
    # if len(self.target_trajectories) == 0:
    #   self.logger.error('No target trajectories for reprojected image saving(). Abort...')
    #   return
    # path handling
    if self.image_save_dir is None:
      start_time = time.localtime()
      date_folder = time.strftime("%Y%m%d", start_time)
      log_dir = os.path.join("/opt/logs", date_folder)
      dir_path = os.path.join(log_dir, dir_name)
      dir_index = 0
      while True:
        dir_name = os.path.join(dir_path, f'{dir_index}')
        if not os.path.exists(dir_name):
          os.makedirs(dir_name)
          break
        dir_index += 1
      self.image_save_dir = dir_name
    
    sector_saved_count = self.sector_cnt_dict[sector.get_sector_id()]
    filename = f'{sector.get_sector_id()}_seq_{sector_saved_count}.png'
    save_path = os.path.join(self.image_save_dir, filename)
    self.sector_cnt_dict[sector.get_sector_id()] += 1

    # get filename
    trajectories = np.array(self.target_trajectories, dtype=trajectory_set.get_dtype())
    image = self.mot3d_visualizer.draw_trajectories_on_image(self.frame, trajectories, None, monotone=True)
    cv2.imwrite(save_path, image)
    
    self.logger.info(f'saving reprojected image to {save_path}')

  def initialize(self):
    self.frame_cached = None
    self.target_cached = {}
    self.frame = None
    self.target_trajectories = []
    self.logger.info('LoggerReprojectedTargets initialized')

class MOTbyYOLO(BehaviorTreeServerNodeV2):
  repo = 'mf_perception'
  node_name = 'mot_by_bbox_bt_server_node'
  def __init__(self, trajectory_visualizer, run_mode = 'server', ):
    super().__init__(run_mode)

    # init queue for frame generation (data synchronizing)
    self.rgb_queue              = mot_dtypes.MfMotImageQueue(P.image_queue_size)
    self.depth_queue            = mot_dtypes.MfMotImageQueue(P.image_queue_size)
    self.idetect_array_queue    = mot_dtypes.MfMotImageBBoxDataArrayQueue(P.light_data_queue_size)
    self.odom_queue             = mot_dtypes.MfMotOdometryQueue(P.light_data_queue_size)
    self.last_odom = None
    self.synced_queue = []

    # initialization
    self.bridge           = CvBridge()
    self.frame_generator  = mot_frame_generator.FrameGeneratorV2()
    self.rgb_K = P.rgb_K
    self.rgb_D = P.rgb_D
    self.depth_K = P.depth_K
    self.depth_D = P.depth_D
    if P.odom_source == 'tf':
      self.tf_buffer        = Buffer()
      self.tf_listener      = TransformListener(self.tf_buffer, self)

    self.rosTF = ROS2TF(self)

    # 3d object detection
    self.det3d_creator = m_fa.Detection3DCreaterBBox(**P.det3d_params)

    # trajectory management (association and lifecycle management)
    self.cost_metric = m_fa.CostMetrics(**P.cost_metric_params)
    self.trajectory_manager = m_fa.Trajectory3DManager(**P.trajectory_params)

    # debug members
    self.trajectory_visualizer = trajectory_visualizer

    # subscribers
    if P.rgb_topic.endswith('compressed'):
      rgb_msg_type = CompressedImage
      rgb_cb = self.compressed_rgb_callback
    else:
      rgb_msg_type = Image
      rgb_cb = self.rgb_callback
    if P.depth_topic.endswith('compressedDepth'):
      depth_msg_type = CompressedImage
      depth_cb = self.compressed_depth_callback
    else:
      depth_msg_type = Image
      depth_cb = self.depth_callback
    if P.odom_source == 'topic':
      self.add_async_subscriber(Odometry, P.odom_topic, self.odometry_callback, 10)
    elif P.odom_source == 'tf':
      self.add_async_timer(P.tf_timer_period_seconds, self.tf_timer)

    self.rgb_lock = Lock()
    self.depth_lock = Lock()
    self.odom_lock = Lock()
    self.yolo_lock = Lock()
    self.traj_lock = Lock()
    self.bt_lock = Lock()

    self.add_async_subscriber(rgb_msg_type, P.rgb_topic, rgb_cb, 10)
    self.add_async_subscriber(depth_msg_type, P.depth_topic, depth_cb, 10)
    self.add_async_subscriber(CameraInfo, P.rgb_info_topic, self.rgb_info_callback, 10)
    self.add_async_subscriber(CameraInfo, P.depth_info_topic, self.depth_info_callback, 10)
    self.add_async_subscriber(YoloOutArray, P.bbox_topic, self.yolo_callback, 10)

    self.add_async_timer(P.dt_mot_timer, self.mot_timer)
    self.mf_logger.info(f'##################### mot_by_bbox_bt_server_node start{P.base_frame} ##########################')
    # health checker members
    self.health_check_lock = Lock()
    self.n_synced_data_1seconds = 0
    self.n_det3ds_1seconds = 0
    self.dt_trajectory_update = []
    self.dt_update_one = []
    self.add_async_timer(1, self.health_timer)
    self.mark_heartbeat(0)

    # behaviour tree related
    self.last_traj_label_fid_list = [] # last mot result
    self.waypoint_queue = [] # waypoint queue for manipulator
    self.stamp_mot_pub = self.create_publisher(MarkerArray, '/mf_perception/stamp_mot_markers', 1)

    # logger reprojected targets
    self.logger_reprojected_targets = LoggerReprojectedTargets(self.trajectory_visualizer, self.mf_logger)


    self.mf_logger.info('MOT by YOLO initialized')

  
  @BehaviorTreeServerNodeV2.available_action()
  def clear_trajectory_db(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    with self.traj_lock:
        self.trajectory_manager.reset_db()
        # Clear related data as well
        self.last_traj_label_fid_list = []
        self.waypoint_queue = []
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}
  
  @BehaviorTreeServerNodeV2.available_action()
  def disable_mot_pruning(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    with self.traj_lock:
        self.trajectory_manager.disable_pruning()
        self.trajectory_manager.reset_db()
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}

  @BehaviorTreeServerNodeV2.available_action()
  def enable_mot_pruning(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    with self.traj_lock:
        self.trajectory_manager.enable_pruning()
        self.trajectory_manager.reset_db()
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}
  
  @BehaviorTreeServerNodeV2.available_action()
  def log_mark_frame(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    self.logger_reprojected_targets.set_frame()
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}


  @BehaviorTreeServerNodeV2.available_action()
  def log_mark_targets(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    if self.run_mode == 'server':
      target_frame_id = input_dict.get('target_frame_id', None)
      if target_frame_id is None:
        self.mf_logger.error("No target frame id. Maybe query_target_frame_id failure?")
        return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}
      self.logger_reprojected_targets.add_target(target_frame_id)
    elif self.run_mode == 'standalone':
      trajectory_set = self.trajectory_manager.get_trajectory_set()
      valid_trajectories = trajectory_set.get_trajectories_by_state(
        m_fa.Trajectory3DSet.LIFE_CONFIRMED, min_count_thresh=P.debug_trajectory_min_det3d_count)
      traj_ls = [t for t in valid_trajectories]
      self.logger_reprojected_targets.target_trajectories(traj_ls)
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}


  @BehaviorTreeServerNodeV2.available_action()
  def log_save_reprojected_image(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    self.logger_reprojected_targets.save_reprojected_image(
      target_sector, 'reprojected_images', self.trajectory_manager.get_trajectory_set())
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}

  @BehaviorTreeServerNodeV2.available_action()
  def log_init_data(self,
                    input_dict: Optional[Dict],
                    target_sector: Sector,
                    target_work: ALLOWED_WORKS):
    self.logger_reprojected_targets.initialize()
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {}

  def health_timer(self):
    if not P.print_mot_health_check:
      return 
    with self.health_check_lock:
      mean_dt = np.mean(self.dt_trajectory_update) if len(self.dt_trajectory_update) else 0
      mean_dt_mot = np.mean(self.dt_update_one) if len(self.dt_update_one) else 0
      self.mf_logger.info(f'n_synced_data_1seconds: {self.n_synced_data_1seconds}')
      self.mf_logger.info(f'n_det3ds_1seconds: {self.n_det3ds_1seconds}')
      self.mf_logger.info(f'mean dt for trajectory update: {mean_dt}')
      self.mf_logger.info(f'mean dt for update mot {mean_dt_mot}')
      self.n_synced_data_1seconds = 0
      self.n_det3ds_1seconds = 0
      self.dt_trajectory_update = []
      self.dt_update_one = []

  def mot_timer(self):
    # sync data
    t_mot_start = time.time()
    with self.rgb_lock:
      with self.depth_lock:
        with self.odom_lock:
          with self.yolo_lock:
            is_success, synced = \
              self.frame_generator.sync_rgbd_odom_det2ds(
                self.rgb_queue,
                self.depth_queue,
                self.odom_queue,
                self.idetect_array_queue,
                max_allowed_rgbd_timediff_ms=P.max_allowed_rgbd_timediff_ms,
                max_allowed_odom_time_diff_ms=P.max_allowed_odom_time_diff_ms)
            if is_success:
              self.n_synced_data_1seconds += len(synced)
            else:
              if P.verbose_sync:
                self.mf_logger.info(f'queue length of rgb, depth, odom, yolo: {len(self.rgb_queue)}, {len(self.depth_queue)}, {len(self.odom_queue)}, {len(self.idetect_array_queue)}')
                error_code = synced
                self.mf_logger.info(f'sync_rgbd_and_interpolate_odom failed: {self.frame_generator.get_error_string(error_code)}')
              return
            
    self.logger_reprojected_targets.set_frame_cached(synced[-1])
            
    with self.traj_lock:
      # create 3D object detection
      det3ds_2darray = self.create_det3ds(synced)
      if len(det3ds_2darray) and P.debug_det3d:
        # publish det3d on an image
        last_det3ds = det3ds_2darray[-1]
        last_synced = synced[-1]
        rgb_cv = last_synced.rgb.image
        self.trajectory_visualizer.publish_det3d_on_image(rgb_cv, last_det3ds)
        image_size_wh = (rgb_cv.shape[1], rgb_cv.shape[0])

        # publish camera tf
        self.trajectory_visualizer.publish_camera_fov_from_frame(
          last_synced.odom, image_size_wh,
          P.depth_max_mm/1000., last_synced.depth.K, frame_id=P.base_frame)

        # publish det3d as markers
        self.trajectory_visualizer.publish_det3ds_as_markers(last_det3ds, P.base_frame)

      total_det3ds = sum([len(det3ds) for det3ds in det3ds_2darray])
      self.n_det3ds_1seconds += total_det3ds

      # update trajectory
      self.update_trajectory(det3ds_2darray)

      # manipulator adapter
      if P.use_manipulator_adapter:
        with self.bt_lock:
          self.last_traj_label_fid_list = []
          traj_set = self.trajectory_manager.get_trajectory_set()
          traj_det3ds_label_list = traj_set.get_confirmed_traj_det3ds_label_list()
          for traj, _, label in traj_det3ds_label_list:
            prefix = 'i hope you do not see me'
            if label == 7:
              prefix='strawberry'
            elif label == 5:
              prefix='flower'
            fid = manipulator_adapter.pub_tf_for_manipulator(self.rosTF, traj, P.base_frame, target_prefix=prefix)
            self.last_traj_label_fid_list.append((traj, label, fid))
            self.logger_reprojected_targets.set_target_cached(fid, traj)
            # self.mf_logger.info(f"[debug-mot_timer] target_frame_id: {fid}, label: {label}, traj: {traj['xyzwlh'][:3]}")

      # debug
      last_frame = synced[-1]
      if P.debug_trajectory_mode == 'simple':
        # self.debug_trajectories(last_frame)
        self.debug_reliable_trajectories(last_frame)

      elif P.debug_trajectory_mode == 'verbose':
        t1 = time.time()
        self.debug_reliable_trajectories(last_frame)
        self.debug_unreliable_trajectories(last_frame)
        self.mf_logger.info(f'debug_reliable_trajectories: {time.time() - t1}')

    t_mot_done = time.time()
    self.dt_update_one.append(t_mot_done - t_mot_start)

  @BehaviorTreeServerNodeV2.available_action()
  def get_next_waypoint(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    if len(self.waypoint_queue) == 0:
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, {'target_xyz': (), 'waypoint_frame_id': P.base_frame}

    with self.bt_lock:
      next_waypoint = self.waypoint_queue.pop(0)
    # next_waypoint = list(*next_waypoint)
    next_list = [float(x) for x in next_waypoint]
    # next_waypoint_list = json.dumps(next_list)
    self.mf_logger.info(f"[debug-get_next_waypoint] {len(self.waypoint_queue)} | next_waypoint: {next_waypoint}, next_list: {next_list}")
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {'target_xyz': tuple(next_list), 'waypoint_frame_id': P.base_frame}

  @BehaviorTreeServerNodeV2.available_action()
  def query_target_frame_id(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    if len(self.last_traj_label_fid_list) == 0:
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, {'target_frame_id': None, 'dist_to_target': None}

    if target_work is not None:
      work_type = target_work.work_type
    else:
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, {'target_frame_id': None, 'dist_to_target': None}

    if work_type == 'pollination':
      target_label = 5
    elif work_type == 'harvesting':
      target_label = 7
    else:
      # self.handle_bt_error(f'Unknown work type: {work_type}')
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, {'target_frame_id': None, 'dist_to_target': None}

    fid_of_closest = None
    dist_to_closest = None
    last_xyz_camera = self.last_odom.pose
    for traj, label, fid in self.last_traj_label_fid_list:
      if label != target_label: continue

      # xyz_target = traj['xyzwlh'][:3]
      # dist_to_cam = np.linalg.norm(np.array(xyz_target) - np.array(last_xyz_camera), 2)
      # self.mf_logger.info(f"[debug-query_target_frame_id] traj: {traj}, label: {label}, fid: {fid}, xyz_target : {traj['xyzwlh'][:3]}, last_xyz_camera: {last_xyz_camera}")
      xyz_target = [traj['xyzwlh'][0], traj['xyzwlh'][2]]
      dist_to_cam = np.linalg.norm(np.array(xyz_target) - np.array([last_xyz_camera[0], last_xyz_camera[2]]), 2)
      if dist_to_closest is None or dist_to_closest > dist_to_cam:
        dist_to_closest = float(dist_to_cam)
        fid_of_closest = fid
   
    self.mf_logger.info(f"fid_of_closest: {fid_of_closest} (type: {type(fid_of_closest)}), dist_to_closest: {dist_to_closest} (type: {type(dist_to_closest)})")

    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {'target_frame_id': fid_of_closest, 'dist_to_target': dist_to_closest}


  @BehaviorTreeServerNodeV2.available_action()
  def stamp_mot(self,
              input_dict: Optional[Dict],
              target_sector: Sector,
              target_work: ALLOWED_WORKS):
    # 1. sort tracks with the condition (predefined condition. Consult with the manipulator team...)
    # 2. create waypoint queue

    # 0. check if pollination or harvesting
    if target_work is not None:
      work_type = target_work.work_type
    else:
      # self.handle_bt_error('context is None. Please check the behavior tree.')
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, None

    if work_type == 'pollination':
      target_label = 5
    elif work_type == 'harvesting':
      target_label = 7
    else:
      # self.handle_bt_error(f'Unknown work type: {work_type}')
      return BehaviorTreeStatus.TASK_STATUS_FAILURE, None

    xyz_list = []
    for traj, label, _ in self.last_traj_label_fid_list:
      if label == target_label:
      # if label in [5, 7]:
        xyz_list.append(traj['xyzwlh'][:3])


    ## pre a hard coding -
    # our rule: visit target which is closer to farm x origin first
    # -y axis of 'manipulator_base_link' is x axis of the farm frame
    # therefore, trajectory with biggest y need to visited first

    xyz_list.sort(key=lambda x: x[0], reverse=True) # rainbow...

    # xyz_list.sort(key=lambda x: x[1], reverse=True) # UR...
    # self.mf_logger.info(f'[debug-stamp_mot]xyz_list : {len(xyz_list)}, xyz_list: {xyz_list}')
    with self.bt_lock:
      self.waypoint_queue = xyz_list
    self.debug_stamp_mot(xyz_list)
    return BehaviorTreeStatus.TASK_STATUS_SUCCESS, {'debug_msg': 'stamp_mot completed'}

  def debug_stamp_mot(self, xyz_list):
    marker_array = MarkerArray()
    for i, xyz in enumerate(xyz_list):
      marker = Marker()
      marker.header.frame_id = P.base_frame
      marker.header.stamp = self.get_clock().now().to_msg()
      marker.ns = 'mot'
      marker.id = i
      marker.type = Marker.SPHERE
      marker.action = Marker.ADD
      marker.pose.position.x = float(xyz[0])
      marker.pose.position.y = float(xyz[1])
      marker.pose.position.z = float(xyz[2])
      marker.scale.x = float(P.cost_metric_params['max_allowed_cube_dist_l2'])
      marker.scale.y = float(P.cost_metric_params['max_allowed_cube_dist_l2'])
      marker.scale.z = float(P.cost_metric_params['max_allowed_cube_dist_l2'])
      marker.color.a = 1.0
      marker.color.r = 0.0
      marker.color.g = 1.0
      marker.color.b = 0.0
      marker_array.markers.append(marker)

      # text marker
      marker = Marker()
      marker.header.frame_id = P.base_frame
      marker.header.stamp = self.get_clock().now().to_msg()
      marker.ns = 'mot'
      marker.id = i + 1000
      marker.type = Marker.TEXT_VIEW_FACING
      marker.action = Marker.ADD
      marker.pose.position.x = float(xyz[0])
      marker.pose.position.y = float(xyz[1])
      marker.pose.position.z = float(xyz[2]) + float(P.cost_metric_params['max_allowed_cube_dist_l2'])
      marker.scale.z = 0.1
      marker.color.a = 1.0
      marker.color.r = 1.0
      marker.color.g = 1.0
      marker.color.b = 1.0
      marker.text = str(i)
      marker_array.markers.append(marker)
    self.stamp_mot_pub.publish(marker_array)

  def debug_trajectories(self, last_frame):
    trajectory_set = self.trajectory_manager.get_trajectory_set()
    trajectories = trajectory_set.get_live_trajectories()
    if len(trajectories) == 0:
      return

    labels = trajectory_set.get_labels(trajectories)
    self.trajectory_visualizer.publish_trajectories_on_image(
      last_frame, trajectories, labels)

    self.trajectory_visualizer.publish_trajectories_as_markers(
      trajectories, labels, P.base_frame)

  def debug_reliable_trajectories(self, last_frame):
    trajectory_set = self.trajectory_manager.get_trajectory_set()
    valid_trajectories = trajectory_set.get_trajectories_by_state(
      m_fa.Trajectory3DSet.LIFE_CONFIRMED, min_count_thresh=P.debug_trajectory_min_det3d_count)
    # if len(valid_trajectories) == 0:
    #   return

    labels = trajectory_set.get_labels(valid_trajectories)
    self.trajectory_visualizer.publish_valid_trajectories_on_image(
      last_frame, valid_trajectories, labels)
    # self.trajectory_visualizer.publish_valid_trajectories_as_markers( # need to improve performance
    #   valid_trajectories, labels, P.base_frame)

  def debug_unreliable_trajectories(self, last_frame):
    trajectory_set = self.trajectory_manager.get_trajectory_set()
    unreliable_trajectories = trajectory_set.get_trajectories_by_state(
      m_fa.Trajectory3DSet.LIFE_ALIVE, min_count_thresh=P.debug_trajectory_min_det3d_count)
    if len(unreliable_trajectories) == 0:
      return

    labels = trajectory_set.get_labels(unreliable_trajectories)
    self.trajectory_visualizer.publish_unreliable_trajectories_on_image(
      last_frame, unreliable_trajectories, labels)
    # self.trajectory_visualizer.publish_unreliable_trajectories_as_markers( # need to improve performance
    #   unreliable_trajectories, labels, P.base_frame)


  def update_trajectory(self, det3ds_list):
    for det3ds_in_a_frame in det3ds_list:
      ts = time.time()
      _, _ = self.trajectory_manager.update_with_det3ds_in_one_frame(
        det3ds_in_a_frame, self.cost_metric)
      self.dt_trajectory_update.append(time.time() - ts)


  def publish_det3d_on_image(self, cv_img, det3ds):
    pass

  def publish_det3d_as_markers(self, det3ds):
    pass

  def create_det3ds(self, synced_frames):
    detections3d_world = []
    for frame_idx, frame in enumerate(synced_frames):
      mf_depth_data   = frame.depth
      mf_det2ds_data  = frame.detections
      mf_odom_data    = frame.odom

      depth_cv = mf_depth_data.image
      depth_cam_K = np.array(mf_depth_data.K, np.float32).reshape(3, 3)

      # preprocess depth
      depth_cv_preprocessed = m_fa.preprocess_depth(
        depth_cv, P.depth_min_mm, P.depth_max_mm, P.depth_unit)

      # tf
      pose = mf_odom_data.pose
      quat = mf_odom_data.orientation
      rot_mat = Rotation.from_quat(quat).as_matrix()

      transform_world_to_cam = np.zeros((4, 4), np.float32)
      transform_world_to_cam[:3, :3] = rot_mat
      transform_world_to_cam[:3, 3] = pose
      transform_world_to_cam[3, 3] = 1

      # create 3D detection
      detections3d_world.append([])
      for det2d in mf_det2ds_data:
        success, det3d = self.det3d_creator.create(
          det2d, depth_cv_preprocessed, depth_cam_K, None)
        if not success:
          if P.detection3d_verbose:
            self.mf_logger.info(f'det3d generation failed: {det3d}')
          continue
        det3d.add_metadata(det2d, '', -1)
        det3d = m_fa.convert_with_tf(det3d, transform_world_to_cam, P.base_frame)
        detections3d_world[-1].append(det3d)

    return detections3d_world

  def add_cv_image_to_queue(self, queue, sec, nsec, cv_image, K, D, lock):
    if self.rgb_K is None or self.rgb_D is None:
      self.mf_logger.info('no rgb camera info!')
      return
    if self.depth_K is None or self.depth_D is None:
      self.mf_logger.info('no depth rgb camera info!')
      return
    with lock:
      queue.add_image(sec, nsec, cv_image, K, D)

  def rgb_callback(self, msg):
    rgb_cv = self.bridge.imgmsg_to_cv2(msg)
    self.add_cv_image_to_queue(
      self.rgb_queue,
      msg.header.stamp.sec,
      msg.header.stamp.nanosec,
      rgb_cv,
      self.rgb_K,
      self.rgb_D,
      self.rgb_lock)

  def compressed_rgb_callback(self, msg):
    rgb_cv = self.bridge.compressed_imgmsg_to_cv2(msg)
    self.add_cv_image_to_queue(
      self.rgb_queue,
      msg.header.stamp.sec,
      msg.header.stamp.nanosec,
      rgb_cv,
      self.rgb_K,
      self.rgb_D,
      self.rgb_lock)

  def rgb_info_callback(self, msg):
    self.rgb_K = np.array(msg.k, np.float32).reshape(3, 3)
    self.rgb_D = np.array(msg.d, np.float32)
    # self.destroy_subscription(self.get_subscriber(P.rgb_info_topic))

  def depth_callback(self, msg):
    depth_cv = self.bridge.imgmsg_to_cv2(msg)
    self.add_cv_image_to_queue(
      self.depth_queue,
      msg.header.stamp.sec,
      msg.header.stamp.nanosec,
      depth_cv,
      self.depth_K,
      self.depth_D,
      self.depth_lock)

  def compressed_depth_callback(self, msg):
    np_arr = np.frombuffer(msg.data, np.uint8)
    depth_cv = cv2.imdecode(np_arr[12:], cv2.IMREAD_UNCHANGED)
    self.add_cv_image_to_queue(
      self.depth_queue,
      msg.header.stamp.sec,
      msg.header.stamp.nanosec,
      depth_cv,
      self.depth_K,
      self.depth_D,
      self.depth_lock)

  def depth_info_callback(self, msg):
    self.depth_K = np.array(msg.k, np.float32).reshape(3, 3)
    self.depth_D = np.array(msg.d, np.float32)
    # self.destroy_subscription(self.get_subscriber(P.depth_info_topic))

  def yolo_callback(self, msg):
    sec, nsec = msg.header.stamp.sec, msg.header.stamp.nanosec
    detections = []
    for out in msg.yolo_out_array:
      score = out.score
      label = out.label
      tlbr = out.tlbr
      tlwh = np.array([tlbr[0], tlbr[1], tlbr[2]-tlbr[0], tlbr[3]-tlbr[1]]).astype(np.float32)

      d = mot_dtypes.MfMotImageBBoxData(sec, nsec, tlwh, score, label)
      detections.append(d)

    with self.yolo_lock:
      self.idetect_array_queue.add_image_bbox_array(sec, nsec, detections)

  def tf_timer(self):
    try:
      transform = self.tf_buffer.lookup_transform(
        P.base_frame,
        P.body_frame,
        rclpy.time.Time())
    except Exception as e:
      if P.verbose_sync:
        self.mf_logger.info(f'Error in odometry_callback: {e}')
      return

    sec, nsec = transform.header.stamp.sec, transform.header.stamp.nanosec
    if len(self.odom_queue):
      ts_cur = mot_common.Timestamp(sec, nsec)
      ts_last = self.odom_queue[-1].ts
      if ts_cur == ts_last:
        return

    pose = np.array([transform.transform.translation.x,
                     transform.transform.translation.y,
                     transform.transform.translation.z]).astype(np.float32)
    orientation = np.array([transform.transform.rotation.x,
                            transform.transform.rotation.y,
                            transform.transform.rotation.z,
                            transform.transform.rotation.w]).astype(np.float32)

    with self.odom_lock:
      self.odom_queue.add_odometry(sec, nsec, pose, orientation, None, None)
      self.last_odom = self.odom_queue[-1]

  def odometry_callback(self, msg):
    sec, nsec = msg.header.stamp.sec, msg.header.stamp.nanosec
    pose = np.array([msg.pose.pose.position.x,
                     msg.pose.pose.position.y,
                     msg.pose.pose.position.z]).astype(np.float32)
    orientation = np.array([msg.pose.pose.orientation.x,
                            msg.pose.pose.orientation.y,
                            msg.pose.pose.orientation.z,
                            msg.pose.pose.orientation.w]).astype(np.float32)
    velocity = np.array([msg.twist.twist.linear.x,
                          msg.twist.twist.linear.y,
                          msg.twist.twist.linear.z]).astype(np.float32)
    angular_velocity = np.array([msg.twist.twist.angular.x,
                                 msg.twist.twist.angular.y,
                                 msg.twist.twist.angular.z]).astype(np.float32)
    with self.odom_lock:
      self.odom_queue.add_odometry(sec, nsec, pose, orientation, velocity, angular_velocity)

def main(args=None):
  rclpy.init(args=args)
  trajectory_visualizer =None
  # if P.debug_det3d or P.debug_trajectory_mode is not None:
  trajectory_visualizer = mot3d_visualizer.TrajectoryVisualizer(P.labels_to_names_and_colors)
  
  run_mode = 'server'
  mot_server = MOTbyYOLO(trajectory_visualizer, run_mode)

  # standalone mode is really difficult to implement
  if run_mode == 'server':
    mot_server.start_ros_thread(async_spin=False)
  elif run_mode == 'standalone':
    mot_server.start_ros_thread(async_spin=True)
    while not rclpy.ok():
      mot_server.mf_logger.info('##################### mot_by_bbox_bt_server_node start ##########################')
      time.sleep(1)
    while rclpy.ok():
      mot_server.mf_logger.info('#####################  start ##########################')
      time.sleep(1)



  input('')
  mot_server.destroy_node()
if __name__ == '__main__':
  main()
