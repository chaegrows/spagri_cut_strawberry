...
from mflib.perception.mot.utils import mot3d_visualizer
from mflib.perception.mot.utils import manipulator_adapter
...
# leaf action
from config.common.leaf_actions.mf_perception import get_actions as get_perception_actions

DEFAULT_LOOKUP_SECONDS = 3
BT_LA_N_MAX_TARGET = 2
...
class MOTbyYOLO(BehaviorTreeServerNodeV2):
  class MOTQueueBTServer:
    def __init__(self):
      self.xyz_queue = [] # id: {'trajectory': trajectory, 'label': label}
      # self.query_rule: str = None # ['closest_to_camera']
      # self.id_order = [] # list of id order

      self.run_tf_br_loop = False
      self.interest_trajectory = None

  def __init__(self, leaf_actions, node_name='mot_by_bbox', trajectory_visualizer=None):
    super().__init__(node_name, leaf_actions)
    ...
    # bt server members
    self.mot_queue_bt_server = MOTbyYOLO.MOTQueueBTServer()
    self.is_mot_activated = False
    ...
    self.dt_update_one = []
    # self.add_async_timer(1, self.health_timer) # not used in bt server mode
  ...
  ### leaf actions
  @BehaviorTreeServerNodeV2.available_action('run_mot_nseconds')
  def run_mot_nseconds(self, context: Optional[MfBehaviorTreeContext] = None):
    self.get_logger().info('run_mot_nseconds start...')
    # inputs
    memory_dict = context.get_memory_dict('mf_perception', 'mot_by_bbox_bt_server_node', 'run_mot_nseconds')
    seconds = memory_dict.get('seconds', DEFAULT_LOOKUP_SECONDS)
    clear_queue = memory_dict.get('clear_queue', True)
    query_rule = memory_dict.get('query_rule', 'closest_to_camera')

    if clear_queue == True:
      if query_rule not in ['closest_to_camera']:
        raise ValueError(f'query_rule {query_rule} is not supported: ')

    # clear queue if required
    if clear_queue:
      self.clear_trajectory_db()
      self.mot_queue_bt_server = MOTbyYOLO.MOTQueueBTServer()
      self.mot_queue_bt_server.query_rule = query_rule
      self.mot_queue_bt_server.run_tf_br_loop = False

    # run mot
    self.is_mot_activated = True
    time.sleep(seconds)
    self.is_mot_activated = False

    # copy results
    traj_set = self.trajectory_manager.get_trajectory_set()
    traj_det3ds_label_list = copy.copy(traj_set.get_confirmed_traj_det3ds_label_list())

    # consider model and jobs to select interest trajectory
    target_work = context.target_work
    perception_algorithm  = target_work['perception_algorithm']
    work_type             = target_work['work_type']
    plant_type            = target_work['plant_type']
    if not (perception_algorithm == 'strawberry_harvesting_pollination' and
            (work_type == 'pollination' or work_type == 'harvesting') and
            plant_type == 'strawberry'):
      raise NotImplementedError('Unknown target_work: ', target_work)

    interest_label = None

    if work_type == 'pollination': interest_label = 5
    elif work_type == 'harvesting': interest_label = 7
    else:
      raise NotImplementedError('Unknown work_type: ', work_type)

    self.mot_queue_bt_server.interest_trajectory = \
      [re[0] for re in traj_det3ds_label_list if re[2] == interest_label]

    self.get_logger().info(f'[MOT] # of trajectory: {len(self.mot_queue_bt_server.interest_trajectory)}')
    self.set_status(BehaviorTreeStatus.TASK_STATUS_SUCCESS)
    if len(self.mot_queue_bt_server.interest_trajectory) == 0:
      self.get_logger().info('no interest trajectory found')


  @BehaviorTreeServerNodeV2.available_action('query_target_frame_id')
  def query_target_xyz(self, context: Optional[MfBehaviorTreeContext] = None):
    # 1-1. get prefix of tf
    work_type = context.target_work['work_type']
    tf_prefix = TF_PREFIX_MAP[work_type]

    try:
      # 1-2. pop one trajectory
      trajectory = self.mot_queue_bt_server.interest_trajectory.pop(0)
      self.get_logger().info(f'trajectory!!!!!!!!!!!!!!!!!!: {trajectory}')

      # check query count
      # self.query_count += 1
      # self.get_logger().info(f'query :::::::: {self.query_count}')
      # if self.query_count > BT_LA_N_MAX_TARGET:
      #   self.query_count = 0
      #   raise IndexError
    except IndexError:
      self.get_logger().info('ALL trajectories are queried')
      self.run_tf_br_loop = True
      self.set_status(BehaviorTreeStatus.TASK_STATUS_FAILURE, {'target_frame_id': 'if you see this, something is wrong'})
      return


    # create tf
    trans = trajectory['xyzwlh'][:3]
    target_id = trajectory['trajectory_id_if_confirmed']
    target_frame = f"{tf_prefix}_{target_id}"
    self.get_logger().info(f'target_frame!!!!!!!!!!!!!!!!!!: {target_frame}')

    TF_PUB_LATENCY = 0.1
    # def pub_tf():
    #   while rclpy.ok() and self.run_tf_br_loop:
    #     self.rosTF.publishTF(P.base_frame, target_frame, trans)
    #     time.sleep(TF_PUB_LATENCY)
    MAX_TF_TIME = 3
    start_tf = time.time()
    def pub_tf():
      while rclpy.ok() and (time.time() - start_tf) < MAX_TF_TIME:
        self.rosTF.publishTF(P.base_frame, target_frame, trans)
        time.sleep(TF_PUB_LATENCY)

    self.run_tf_br_loop = True
    threading.Thread(target=pub_tf, daemon=True).start()

    self.set_status(BehaviorTreeStatus.TASK_STATUS_SUCCESS, {'target_frame_id': target_frame})

    # if query_rule == 'closest_to_camera' and len(self.mot_queue_bt_server.interest_trajectory) > 1:
    #   # get latest body xyz
    #   body_pose = self.frame_latest.odom.pose

    #   dists = []
    #   for tr in self.mot_queue_bt_server.interest_trajectory:
    #     dists.append(np.linalg.norm(tr['xyzwlh'][:3] - body_pose))
  ...
  def mot_timer(self):
        ...
        with self.odom_lock:
          with self.yolo_lock:
            ...
            if is_success:
              ...
              if P.verbose_sync:
                self.get_logger().info('queue length of rgb, depth, odom, yolo: ',
                        len(self.rgb_queue), len(self.depth_queue),
                        len(self.odom_queue), len(self.idetect_array_queue))
                error_code = synced
                self.get_logger().info('sync_rgbd_and_interpolate_odom failed: ', \
                      self.frame_generator.get_error_string(error_code))
              return

    if not self.is_mot_activated:
      return
    ...
    if P.debug_trajectory_mode == 'simple':
      ...
      self.get_logger().info('debug_reliable_trajectories:', time.time() - t1)
      ...
      for det2d in mf_det2ds_data:
        ...
        if not success:
          if P.detection3d_verbose:
            self.get_logger().info(f'det3d generation failed:{det3d}')
          continue
  ...
  def add_cv_image_to_queue(self, queue, sec, nsec, cv_image, K, D, lock):
    if self.rgb_K is None or self.rgb_D is None:
      self.get_logger().info('no rgb camera info!')
      return
    if self.depth_K is None or self.depth_D is None:
      self.get_logger().info('no depth rgb camera info!')
      return
    with lock:
...
def main(args=None):
  ...
  mot_server = MOTbyYOLO(
    get_perception_actions(),
  ...
  if run_mode == 'server':
    mot_server.init_server_mode('default.yaml')
    mot_server.get_job_spec().print_data_structure(print_query_keys=True)
