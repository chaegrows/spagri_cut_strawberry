...
class RelocBtServer(BehaviorTreeServerNodeV2):
  ...
  def __init__(self, run_mode):
    ...
    self.calculate_done_flag = False
    self.static_frame_id = 'farm_pcd'
    self.static_child_frame_id = 'map'
    self.static_trans = []
    self.static_rot = []
    self.add_async_timer(5.0, self.pub_static_tf)
    # self.accumulate_and_relocate(None, None, None)


  def pub_static_tf(self):
    if self.calculate_done_flag == True:
      self.ros2TF.publishTF(self.static_frame_id, self.static_child_frame_id, self.static_trans, self.static_rot, static_tf=True)
      self.mf_logger.info('Reloc static tf published.')
    else:
      self.mf_logger.info('Waiting for reloc static tf to be set...')
  ...
  def accumulate_and_relocate(self, input_dict: Optional[Dict], target_sector: Sector, target_work: ALLOWED_WORKS):
    ...
    self.static_trans = T_map_P[:3, 3]
    self.static_rot = R.from_matrix(T_map_P[:3, :3]).as_quat()
    self.calculate_done_flag = True
    self.mf_logger.info(f'static_trans: {self.static_trans}, static_rot: {self.static_rot}')
